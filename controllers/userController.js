...
exports.resendVerificationEmail = async (req, res) => {
  try {
    ...
    // Generate a token for the user
    const token = await jwt.sign({ userId: user._id }, pocess.env.JWT_SECRET, {
      expiresIn: "1hour",
    });
    // configure the link to verify the user
    const link = `${req.protocol}://${req.get(
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message:
        "Reset password initiated, please check your eamil for the reset link ",
    });
  } catch (error) {
    console.error(error);
    if (error instanceof jwt.JsonWebTokenError) {
...
exports.loginUser = async (req, res) => {
  try {
    ...
    if (user.isVerified === false) {
      return res.status(400).json({
        message:
          "account notverified, please check your email for the verification link",
      });
    }
    // generate a token for the user
